...
const userRoleOptions = [
  { name: "مشرف", value: "supervisor" },
  { name: "مدير", value: "admin" },
  { name: "عميل", value: "client" },
];

export default function ModelAddUsersTBody({
  formik,
  uploading,
  fileInputRef,
  handleFileSelect,
      ...
      <tr className="h-[60px]">
          ...
          <div>
            {formik.values.imagePreview ? (
              <div className="relative">
                ...
                <X
                  onClick={removeImage}
                  className="w-4 h-4 text-white rounded-full bg-destructive absolute top-0 right-0 cursor-pointer"
                />
              </div>
            ) : (
        ...
        <td className="bg-[#C8CCCC] border-l-[1px] border-secondary h-[60px]">
          <input
            ...
            className="placeholder:text-center placeholder:text-primary h-full w-full text-center border-0 focus:outline-0 text-primary"
            placeholder="......"
          />
        </td>
