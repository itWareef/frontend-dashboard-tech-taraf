...
// yup schema imports
import { AddNewPaymentSchema } from "@/lib/YupSchemas/StorePage/AddPaymentsSchema";
...
const ModelContent = ({ setTableData, selectedRow, setIsOpen }) => {
  ...
  const initialValues = {
    name: selectedRow.customer_name,
    phone_number: selectedRow.number,
    bill: selectedRow.invoice_value,
    bill_details: selectedRow.invoice_details,
  };
  //------------------------------------------------
  ...
  return (
    <div className="absolute inset-0 backdrop-blur-md z-50 w-full h-full flex flex-col">
      ...
      <div className="w-full z-50 flex flex-col justify-center gap-3 py-5 px-20 ">
        <h1 className="text-5xl font-bold">تعديل المدفوعات</h1>
        <button
...
const EditPaymentModel = ({ setTableData, selectedRow, isOpen, setIsOpen }) => {
  return (
    <>
      {isOpen && (
        <ModelContent
